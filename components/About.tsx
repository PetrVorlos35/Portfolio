    ...
    <section id="about" className="py-28 max-w-6xl mx-auto px-6 md:px-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24">
        ...
        <motion.div {...reveal()}>
          ...
          <p className="text-gray-600 leading-relaxed text-base mb-12 max-w-prose">
            {t.about.bio2}
          </p>

          {/* Skills */}
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-widest mb-8">{t.about.techLabel}</p>
              ...
              {skills.map((group) => (
                <div key={group.label}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {group.items.map((s) => (
                      <span key={s} className="text-xs text-gray-700 border border-gray-200 bg-white px-3 py-1.5 rounded-full hover:border-black hover:bg-black hover:text-white hover:shadow-sm cursor-default transition-all duration-300">
                        {s}
                      </span>
                    ))}
                  </div>
        ...
        <motion.div {...reveal(0.15)}>
          <p className="text-xs text-gray-500 uppercase tracking-widest mb-8">{t.about.pathLabel}</p>
          <div className="relative pl-4 md:pl-6 border-l border-gray-200 ml-2">
            {timeline.map((item, i) => (
              ...
              >
                ...
                <span className="text-xs text-gray-400 font-mono mt-1 w-6 shrink-0 group-hover:text-accent-ink transition-colors duration-300">{item.num}</span>
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-4">
                    <h3 className="text-base font-medium text-black">{item.title}</h3>
                    <span className="text-xs text-gray-600 font-mono whitespace-nowrap bg-gray-50 px-2.5 py-1 rounded-md border border-gray-200">{item.period}</span>
                  </div>
